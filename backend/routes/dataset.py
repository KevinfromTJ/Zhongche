...
@dataset_bp.route('/datasets/<int:dataset_id>/import-frames', methods=['POST'])
def import_frames_from_video(dataset_id):
    """
    从视频帧导入图片到数据集
    ...
    if not frame_ids or not isinstance(frame_ids, list):
        return jsonify({'success': False, 'message': '请提供要导入的帧ID列表'}), 400
    ...
    for frame_id in frame_ids:
        try:
            ...
            if not frame:
                skipped_count += 1
                error_messages.append(f"帧ID {frame_id} 不存在")
                continue
            
            if not os.path.exists(frame['image_path']):
                skipped_count += 1
                error_messages.append(f"帧ID {frame_id} 的图片文件不存在")
                continue
            ...
            # 生成新文件名: v{video_id}_f{frame_idx}_{original_name}
            original_filename = frame['image_filename']
            name, ext = os.path.splitext(original_filename)
            new_filename = f"v{video_id}_f{frame_idx}_{original_filename}"
            ...
            counter = 1
            while os.path.exists(dst_path):
                new_filename = f"v{video_id}_f{frame_idx}_{name}_{counter}{ext}"
                dst_path = os.path.join(dataset_dir, new_filename)
                counter += 1
            
            # 复制文件
            import shutil
            shutil.copy2(frame['image_path'], dst_path)
            ...
            skipped_count += 1
            error_messages.append(f"帧ID {frame_id}: {str(e)}")
            print(f"❌ 导入帧 {frame_id} 失败: {e}")
    
    # 构建响应消息
    message = f"成功导入 {imported_count} 张图片"
    if skipped_count > 0:
        message += f"，跳过 {skipped_count} 张"
    ...
    response_data = {
        ...
        'data': {
            ...
            'total_requested': len(frame_ids)
        }
    }
    
    if error_messages and len(error_messages) <= 10:
        response_data['data']['errors'] = error_messages
